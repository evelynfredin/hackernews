...
<article class="flex py-10 border-b dark:border-primary-500">
    ...
    <!-- Form to show if the user is authenticated and has upvoted -->
    @auth
    @if($post->votedBy(auth()->user()))

    <form action="{{ route('posts.votes', $post) }}" method="POST">
        @csrf
        <!-- If the user has upvoted then the upvote can be deleted -->
        @method('DELETE')
    ...
    <!-- If the authenticated user has not upvoted -->
    @else
    ...
    <!-- Form to show guest users who can't upvote posts -->
    @guest
